...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const profileResponse = await fetch(`${FIRESTORE_USER_URL}/${data.localId}`, {
    ...
    await chrome.storage.session.set({ authSession: { uid: data.localId,
      name: profile.fields?.name?.stringValue || profile.fields?.displayName?.stringValue || data.displayName || data.email, role,
